...
class AvailableVehiclesViewSet(viewsets.ViewSet):
   ...
   # authentication_classes = []
    ...
    def list(self, request):
        num_of_people = int(request.query_params.get('num_of_people', 1))
        date_str = request.query_params.get('date')
        date = parse_date(date_str) if date_str else None

        # Calculate the nearest multiple of 10 greater than or equal to the requested number of people
        nearest_multiple_of_10 = math.ceil(num_of_people / 10) * 10

        # Filter available vehicles based on the calculated nearest multiple of 10
        available_vehicles = Vehicle.objects.filter(capacity__gte=nearest_multiple_of_10)

        # If no available vehicles with capacities up to the nearest multiple of 10, then show all vehicles with capacities greater than the requested number of people
        if not available_vehicles:
            available_vehicles = Vehicle.objects.filter(capacity__gt=num_of_people)
