...
class Aspirant_form(forms.ModelForm):
    ...
    def clean(self):
        ...
        Duration_of_Course=total_clean_data["Duration_of_Course"]
        if not re.match(r'^.*\d.*[a-zA-Z].*$', Duration_of_Course):
            raise forms.ValidationError("Duration of the course must include both numbers and characters")
