...
def delete_view(request,id):
    d=Aspirant.objects.get(id=id)
    d.delete()
    return redirect("/")

def update_view(request,id):
    e=Aspirant.objects.get(id=id)
    form=Aspirant_form(instance=e)
    if request.method == "POST":
        form=Aspirant_form(request.POST,instance=e)
        if form.is_valid():
            form.save()
            return redirect("/")
    return render(request,"application/update.html",{"form":form})