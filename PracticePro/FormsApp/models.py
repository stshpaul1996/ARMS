...
def name_check(value):
    if value.isalpha():
        return value.upper()
    raise ValidationError("name does not contain any numbers and special symbols")

def phone_check(value):
    if len(str(value)) != 10:
        raise ValidationError("Phone number contains only 10 digits")
    return value
...
class Resources(models.Model):
    first_name = models.CharField(max_length=250, validators=[name_check])
    last_name = models.CharField(max_length=250, validators=[name_check])
    email = models.EmailField(max_length=250)
    phone = models.BigIntegerField(validators=[phone_check])
